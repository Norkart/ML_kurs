...
function getStoreMainDir() {
    ...
    if (process.platform === 'win32' && process.env.LOCALAPPDATA) {
        return join(process.env.LOCALAPPDATA, exports.values.myNameIsCapitalized);
    }
    return join(userHome, '.' + exports.values.myNameIs);
}
...
exports.getPackageStoreDir = function (packageName, version) {
    ...
    return path.join(getStorePackagesDir(), packageName, version);
};
exports.getPackageManager = function (workingDir) {
    return fs.existsSync('yarn.lock') ? 'yarn' : 'npm';
};
...
function readPackageManifest(workingDir) {
    var pkg;
    var packagePath = join(workingDir, 'package.json');
    try {
...
function readSignatureFile(workingDir) {
    var pkg;
    var signatureFilePath = join(workingDir, signatureFileName);
    try {
...
exports.readSignatureFile = readSignatureFile;
function writeSignatureFile(workingDir, signature) {
    var pkg;
    var signatureFilePath = join(workingDir, signatureFileName);
    try {
...
exports.writeSignatureFile = writeSignatureFile;
var sortDependencies = function (dependencies) {
    return Object.keys(dependencies).sort().reduce(function (deps, key) {
